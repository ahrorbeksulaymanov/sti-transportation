        ...
        <div>
            ...
            </section>
            {
                ...
                <div className="my-container">    
                    ...
                    <div className="grid grid-cols-6 gap-[30px]">
                        {
                            data?.slice(0, 2).map((item:any, index:number) => (
                                <div data-aos="fade-up"  key={index} className="md:col-span-3 col-span-6">
                                    <img className='w-[100%]' src={PATH_FILE + item.file_url} alt="" />
                                    ...
                                    <p className='md:text-[18px] text-[14px] font-[400] md:leading-[22px] leading-[17px] mb-[26px]'>{item?.description}</p>
                    ...
                    <div className="grid grid-cols-12 gap-x-[20px] gap-y-[70px]">
                        {
                            data?.slice(2).map((item:any, index:number) => (
                                <div data-aos="fade-up"  key={index+3} className="lg:col-span-4 md:col-span-6 col-span-12">
                                    <div className='duration-200 cursor-pointer hover:translate-y-[-10px]'>
                                        ...
                                        <p className='md:text-[16px] text-[14px] font-[400] md:leading-[20px] leading-[17px] mb-[26px]'>{item?.description}</p>
                                        <div className="flex justify-start" style={{alignItems:"center"}}>
                                            {
                                                item?.hashtag?.split("#")?.map((h:string, i:number) => (
                                                    h?.length > 0 ? <span key={i} className='inline-block text-orange rounded-full px-[13px] py-[6px] text-[14px] leading-[14px] mr-[15px]' style={{border:"1px solid #EE3532"}}>#{h}</span> : ""
                    ...
                    <div className='text-center mt-[50px] text-white blog-pagination flex justify-between'>
                        {
                            currentPage > 1?
                        <a onClick={() => {setcurrentPage(currentPage-1); setrefresh(!refresh)}} className='flex hover:translate-x-[-4px]' style={{alignItems:"center", transition:"0.2s"}}>
                            <svg className='mr-[18px] rotate-180' width="16" height="12" viewBox="0 0 16 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <div className='flex'>
                            {[...Array(pageSize)].map((item, index) => ( 
                                <span onClick={() => {setcurrentPage(index+1); setrefresh(!refresh)}} className={`inline-block text-[16px] px-[19px] py-[12px] mx-[2px] cursor-pointer ${currentPage == (index+1) ? "pagination-active" : "" }`}>{index+1}</span> 
                                ) 
                            )}
                        </div>

                        <a onClick={() => {setcurrentPage(currentPage+1); setrefresh(!refresh)}} className='flex hover:translate-x-[4px]' style={{alignItems:"center", transition:"0.2s"}}>Next 
                            <svg className='ml-[18px]' width="16" height="12" viewBox="0 0 16 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M10 12L8.6 10.55L12.15 7H0V5H12.15L8.6 1.45L10 0L16 6L10 12Z" fill="white"/>
                            </svg>
                        </a>
                    </div>
